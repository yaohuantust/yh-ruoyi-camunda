<template>
  <div>
    ...
    <el-button
      ...
      icon="el-icon-back"
      @click="handleCancel"
      v-show="taskId !== '-1' && taskId !== '-2' && type === 'none'"
    >撤销</el-button>
    <el-button
      size="mini"
      type="text"
    ...
    ></demo-form>
    <leave-form
      v-if="row.processKey === 'leave_demo'"
      :row="row"
      :taskId="taskId"
      :taskName="taskName"
      :type="type"
...
<script>
  ...
  import LeaveForm from "@/components/Camunda/ApplyAfter/leaveForm";
    ...
    methods: {
      ...
      handleState: function () {
        ...
        this.$confirm('是否确认' + opt + 'ID为"' + instanceId + '"的流程实例?', "警告", {
          ...
          return request({
            url: '/camunda/process/suspendOrActiveApply',
            method: 'post',
            params: data
          });
        }).then(response => {
      ...
      handleCancel: function () {
        ...
        this.$confirm('是否确认撤销ID为"' + instanceId + '"的流程实例?', "警告", {
          ...
          return request({
            url: '/camunda/process/cancelApply',
            method: 'post',
            params: data
          });
        }).then(response => {
      ...
      handleViewProcessImg: function() {
        ...
        this.src = baseURL + '/camunda/process/read-resource?pProcessInstanceId=' + this.row.instanceId + '&_=' + new Date().getTime();
        this.srcList = [];
      ...
      getHistoryList: function () {
        this.loading = true;
        return request({
          url: '/camunda/process/listHistory',
          method: 'post',
